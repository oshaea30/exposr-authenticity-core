"""
Playwright Fallback Scraper for Real Images
Handles JavaScript-rendered content for public domain image sources
"""
import os
import asyncio
import logging
...
class PlaywrightRealFallbackScraper:
    ...
    async def scrape_source(self, source: str, count: int) -> int:
        """Scrape images from a specific source using Playwright"""
        logger.info(f"🎨 Scraping {source} with Playwright for {count} real images")
        ...
        try:
            ...
            urls = {
                'public_domain_archive': 'https://publicdomainarchive.com',
            ...
            # Navigate to source
            await self.page.goto(urls[source], wait_until="networkidle", timeout=30000)
            logger.info(f"✅ Loaded {source} homepage")
            ...
            logger.info(f"Found {len(image_urls)} potential images from {source}")
            ...
            for i, img_url in enumerate(image_urls):
                ...
                try:
                    ...
                    filename = f"{source}_playwright_{collected:06d}.jpg"
                    
                    if self.download_image(img_url, filename):
                        file_size = os.path.getsize(self.output_dir / filename)
                        self.log_image_metadata(filename, f"{source}_playwright", "public_domain", img_url, file_size)
                        collected += 1
                        logger.info(f"✅ Downloaded {source} image {collected}/{count}")
        ...
        logger.info(f"Collected {collected} images from {source} (Playwright)")
        return collected
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Playwright fallback scraper for real images")
    ...
    parser.add_argument("--source", type=str, required=True, help="Source to scrape")
    parser.add_argument("--stats", action="store_true", help="Show collection statistics")
    
    args = parser.parse_args()
    
    scraper = PlaywrightRealFallbackScraper(args.output)
    ...
    try:
        collected = await scraper.scrape_source(args.source, args.count)
        
        print(f"✅ Successfully collected {collected} real images with Playwright")
